...
class SelectableBoxLayout(RecycleDataViewBehavior, BoxLayout):
	...
	def refresh_view_attrs(self, rv, index, data):
		...
		self.ids['_cantidad'].text = str(data.get('cantidad_carrito', 0))
		self.ids['_precio_por_articulo'].text = str("{:.2f}".format(data['precio']))
		self.ids['_precio'].text = str("{:.2f}".format(data.get('precio_total', 0.0)))
...
class RV(RecycleView):
	...
	def agregar_articulo(self, articulo):
		...
		if self.data:
			...
			print("Nuevo producto, agregándolo al carrito")
			self.data.append(articulo)
	...
	def eliminar_articulo(self):
		...
		if indice >=0:
			...
			articulo_eliminado = self.data.pop(indice)
			self.refresh_from_data()
	...
	def seleccionar_articulo(self):
		...
		if indice>=0:
			...
			if callable(self.agregar_producto):
				...
				print("Artículo enviado al carrito")
			else:
				print("No se seleccionó ningún artículo")
			self.dismiss()
	...
	def mostrar_cambio(self):
		...
		try:
			...
			if cambio >=0:
				...
				self.ids.cambio.text = 'Pago insuficiente'
		except:
...
class VentasWindow(BoxLayout):
	def __init__(self, actualizar_productos_callback, **kwargs):
		super(VentasWindow, self).__init__(**kwargs)
		self.total=0.0
	...
	def eliminar_producto(self):
		articulo = self.ids.rvs.eliminar_articulo()
		if isinstance(articulo, dict):
			menos_precio = articulo.get('precio_total', 0)
			self.total -= menos_precio
			self.ids.sub_total.text = f'Total: ${self.total:2f}'
			print(f'Producto eliminado: {articulo}, Total actualizado: {self.total}')
		else:
			print('No se pudo eliminar el producto correctamente')
	...
	def pagar(self):
		if self.ids.rvs.data:
			...
			self.ids.notificacion_falla.text = 'CARRITO VACIO'

	def pagado(self):
		self.ids.notificacion_exito.text = '¡TRANSACCIÓN EXITOSA!'
