...
class SelectableUsuarioLabel(RecycleDataViewBehavior, BoxLayout):
    ...
    # Manejo seguro de la clave 'nombre'
        self.ids['_nombre'].text = data.get('nombre', 'Sin nombre').title()
    
    # Manejo seguro de otras claves
        self.ids['_username'].text = str(data.get('username', 'Sin username'))
    
    # Manejo seguro del tipo
        tipo = data.get('tipo', '')
        if isinstance(tipo, (int, float)):
            self.ids['_tipo'].text = str('{:.2f}'.format(tipo))
        else:
            self.ids['_tipo'].text = str(tipo)
